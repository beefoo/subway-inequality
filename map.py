...
parser.add_argument('-out', dest="OUTPUT_FILE", default="data/2010_Census_Tracts_Income.geojson", help="Output json file pattern")
a = parser.parse_args()
...
for i, feat in enumerate(geojson["features"]):
    ...
    match = matchCensusTract(cdataLookup, props)
    if match:
        geojson["features"][i]["properties"]["income"] = match[INCOME_KEY]

incomes = [f["properties"]["income"] for f in geojson["features"] if "income" in f["properties"]]
incomeMin, incomeMax = (min(incomes), max(incomes))
...
def getColors():
    ...
    return ['#%02x%02x%02x' % tuple([int(cc*255) for cc in c]) for c in colors]

gcolors = getColors()
ccount = len(gcolors)
propsOut = ["boro_code", "ct2010", "income"]
for i, feat in enumerate(geojson["features"]):
    props = feat["properties"]
    if "income" in props:
        n = norm(props["income"], (incomeMin, incomeMax), limit=True)
        color = gcolors[roundInt(n*(ccount-1))]
        pout = {}
        for key in propsOut:
            if key in props:
                pout[key] = props[key]
        ...
        geojson["features"][i]["properties"] = pout
